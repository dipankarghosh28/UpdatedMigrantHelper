...
class ClothingViewController: UIViewController, CLLocationManagerDelegate {
    @IBOutlet weak var clothingtextview: UITextView!
    @IBOutlet weak var clothingmap: MKMapView!
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        clothingmap.setRegion(region, animated: true)
        self.clothingmap.showsUserLocation = true
    ...
    @IBAction func ClothingNeedHelp(_ sender: Any) {
        //post data to firebase
        var refdb: DocumentReference? = nil
        refdb = db1.collection("clothingaddress").addDocument(data: [
            "clothingddress": globalvariable.myStruct
        ]) { err in
            if let err = err {
        ...
        print("Clothing Clicked GlobalVariable stores->",globalvariable.myStruct)
        ref?.child("clothingaddress").childByAutoId().setValue(globalvariable.myStruct)
    }
    @IBAction func GetLocation(_ sender: Any) {
        ...
        NSLog("Clothing needed here")
        
        ref = Database.database().reference()
        
        databaseHandle = ref?.child("clothingaddress").observe(.childAdded, with: {(snapshot) in
            
            let post = snapshot.value as? String
            if let actualPost = post{
                self.postData.append(actualPost)
                self.clothingtextview.text = actualPost
