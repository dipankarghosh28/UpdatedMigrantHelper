...
class HealthcareViewController: UIViewController, CLLocationManagerDelegate {
    @IBOutlet weak var healthcaretextview: UITextView!
    @IBOutlet weak var healthcaremap: MKMapView!
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        healthcaremap.setRegion(region, animated: true)
        self.healthcaremap.showsUserLocation = true
    ...
    @IBAction func healthcareNeedHelp(_ sender: Any) {
        //post data to firebase
        var refdb: DocumentReference? = nil
        refdb = db1.collection("healthcareaddress").addDocument(data: [
            "healthcareddress": globalvariable.myStruct
        ]) { err in
            if let err = err {
        ...
        print("Healthcare Clicked GlobalVariable stores->",globalvariable.myStruct)
        ref?.child("healthcareaddress").childByAutoId().setValue(globalvariable.myStruct)
    }
    @IBAction func GetLocation(_ sender: Any) {
        ...
        NSLog("Healthcare needed here")
        
        ref = Database.database().reference()
        
        databaseHandle = ref?.child("healthcareaddress").observe(.childAdded, with: {(snapshot) in
            
            let post = snapshot.value as? String
            if let actualPost = post{
                self.postData.append(actualPost)
                self.healthcaretextview.text = actualPost
