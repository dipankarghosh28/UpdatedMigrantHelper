...
class FoodViewController: UIViewController , CLLocationManagerDelegate {
   ...
   // var ref: DatabaseReference! = Database.database("https://needhelp-ed995.firebaseio.com/").reference()
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
     ...
     self.foodmap.showsUserLocation = true
    ...
    @IBAction func FoodNeedHelp(_ sender: Any) {
        //post data to firebase
        var refdb: DocumentReference? = nil
        refdb = db1.collection("foodaddress").addDocument(data: [
            "foodaddress": globalvariable.myStruct
        ]) { err in
            if let err = err {
        ...
        ref?.child("foodaddress").childByAutoId().setValue(globalvariable.myStruct)
    }
    @IBAction func GetLocation(_ sender: Any) {
        ...
        NSLog("Food needed here")
        
        ref = Database.database().reference()
        
        databaseHandle = ref?.child("foodaddress").observe(.childAdded, with: {(snapshot) in
            
            let post = snapshot.value as? String
            if let actualPost = post{
                self.postData.append(actualPost)
                self.foodtextview.text = actualPost
